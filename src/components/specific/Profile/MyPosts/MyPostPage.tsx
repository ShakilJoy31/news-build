...
const MyPostsPage = () => {
  ...
  const { data, isLoading: getLoading } = useGelAllPostsByAuthorQuery(id);
  // console.log(data?.posts);

  useEffect(() => {
    if (data && data?.posts && !getLoading) {
      const postById = data?.posts.filter(
        (post: Post) => post?.authorId === id
      );
      setPosts(postById);
                ...
                <tr key={post?.id} className="border-b bg-white">
                  ...
                  <td className="">
                    <div className="flex gap-1 items-center justify-center">
                      <Link href={`/my-posts/edit-post?slug=${post?.slug}`}>
